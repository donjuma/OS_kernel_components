...
int shell_init(char *envs_array[]){
    int n = 0;
    int i = 0;

    if (getcwd(cwd, sizeof(cwd)) == NULL){
        perror("getcwd() error in shell initialization");
    }
    char * mypath = getenv("PATH");
    strcat(mypath, ":");
    strcat(mypath, cwd);
    char **temp = NULL;
    char *tokens = strtok(mypath, ":");

    while(tokens){
        temp = realloc(temp, sizeof(char*) * ++n);
        if (temp == NULL){
            return -1;
        }else{
            temp[n-1] = toens;
    ...
    temp = realloc(temp, sizeof(char*) * (n+1));
    temp[n] = 0;
    paths = temp;
    return 1;
}
